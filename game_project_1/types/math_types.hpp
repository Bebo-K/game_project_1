...
struct AABB
{
    ...
    inline void SetNorth(float top)
    {
        hi_corner.z = top;
        if (lo_corner.z > hi_corner.z)
        {
            lo_corner.z = hi_corner.z;
        }
    }

    inline float South() { return lo_corner.z; }
    inline float South(vec3 offset) { return lo_corner.z + offset.z; }
    inline void SetSouth(float bottom)
    {
        lo_corner.z = bottom;
        if (hi_corner.z < lo_corner.z)
        {
            hi_corner.z = lo_corner.z;
        }
    }

    inline float East() { return hi_corner.x; }
    inline float East(vec3 offset) { return hi_corner.x + offset.x; }
    inline void SetEast(float east)
    {
        hi_corner.x = east;
        if (lo_corner.x > hi_corner.x)
        {
            lo_corner.x = hi_corner.x;
        }
    }

    inline float West() { return lo_corner.x; }
    inline float West(vec3 offset) { return lo_corner.x + offset.x; }
    inline void SetWest(float west)
    {
        lo_corner.x = west;
        if (hi_corner.x < lo_corner.x)
        {
            hi_corner.x = lo_corner.x;
        }
    }
    ...
    {
        hi_corner.y = top;
        if (lo_corner.y > hi_corner.y)
        {
            lo_corner.y = hi_corner.y;
        }
    }
    ...
    {
        lo_corner.y = bottom;
        if (hi_corner.y < lo_corner.y)
        {
            hi_corner.y = lo_corner.y;
        }
    }
