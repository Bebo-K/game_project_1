...
NetTarget::~NetTarget(){//:~connection(),~reliable_buffer(),~inbound_buffer(),~outbound_buffer()
    last_success=0;
    latency=0;
    ...
    for(InboundMultipartPayload* multipart: multipart_buffer){multipart->Clear();}
}
...
void NetTarget::Clear(){
    ...
    for(InboundMultipartPayload* payload: multipart_buffer){payload->Clear();}
    for(int i=0;i<RELIABLE_ID_HISTORY;i++){ackd_reliables[i]=0;}
    for(int i=0;i<RELIABLE_ID_HISTORY;i++){sent_reliables[i]=0;}
}
...
bool NetTarget::OnRecieve(Payload payload){ 
    if(payload.IsReliable()){
        if(!SendAck(payload.id)){return false;}//Already acknowleged- don't forward
    }
    if(payload.type == PacketID::OKAY){//Acknowledgement- don't forward
        RemoveFromReliableBuffer(Packet::OKAY(payload).ack_id);
        return false;
    }
    if(payload.type == PacketID::ACPT){//Special ack from server accept request- Still forward
        RemoveFromReliableBuffer(Packet::ACPT(payload).ack_id);
        return true;
    }
    if(payload.type == PacketID::NOPE){//Disconnect- Still forward so target can handle it as well.
        connection.SetState(Connection::DISCONNECTED,wstr::new_copy(Packet::NOPE(payload).reason_buffer));
        return true;
    }
    if(payload.type == PacketID::PING){//Ping- We just use these for measuring latency and keeping alive connections. Don't forward.
    ...
    while(inbound_buffer.Read((byte*)&datagram)){
        if(datagram.IsMultipart()){
            ...
            if(mp_payload->IsFullyAssembled()){
                ...
                if(OnRecieve(finished_payload)){return finished_payload;}
            }
        }
        else{
            Payload payload = datagram.ToPayload();
            if(OnRecieve(payload)){return payload;}
        }
    }
...
bool NetTarget::SendAck(int ack_id){//always send back an OKAY packet, returns true if not acknowledged before
