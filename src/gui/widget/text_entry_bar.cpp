...
void TextEntryKeyboardOverlay::OnSelect(Widget* text_entry){
    if(selected == back_pos){
        ...
        if(wstr::len(entry.string)> 0){//Validate
            ...
            text_entry->texts[0]->SetString(entry.string);
        }
    }
    else{
        AddSelectedChar();
...
bool TextEntryKeyboardOverlay::OnInput(Widget* w, Input::Event event_type){
    ...
    switch(event_type){
        ...
        case Input::Event::PC_Cursor:{
            ...
            point_i cursor_letter_index = Controller::GetPCCursor() - letter_origin;
        ...
        case Input::Event::PC_Escape:{
            text_entry->selectable->locked=false;
            this->entry.SetString(text_entry->texts[0]->string);
            Input::SetKeyboardTypingOnly(false);
            this->active=false;
            handled= true;
            break;
...
TextEntryBar::TextEntryBar(char* label,wchar* text):overlay(){
